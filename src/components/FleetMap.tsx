...
export default function FleetMap({focusedIds,onSelect,vehicles=[],traffic=true}:{focusedIds:string[];onSelect:(id:string)=>void;vehicles?:LiveVehicle[];traffic?:boolean}){
 ...
 const visibleRoutes=useMemo(()=>routes.filter(r=>focusedIds.length===0||vehicles.some(v=>focusedIds.includes(v.id)&&v.plate_number===r.id)),[routes,vehicles,focusedIds]);
 ...
 <Source id="routes" type="geojson" data={visibleData} lineMetrics>
  ...
  {vehicles.length===0&&visibleRoutes.map((r,i)=>{const p=pointAt(r.points,(r.progress+tick*.00016*(1+i*.08))%1);return <Marker key={r.id} longitude={p.lng} latitude={p.lat}><button className="vehicle-pin" style={{"--pin":r.color} as React.CSSProperties}><Navigation style={{transform:`rotate(${90-p.angle}deg)`}}/><span>{r.id}</span></button></Marker>})}
  {visibleVehicles.map(v=><Marker key={v.id} longitude={v.last_lng!} latitude={v.last_lat!}><button className={focusedIds.includes(v.id)?"vehicle-pin selected live":"vehicle-pin live"} style={{"--pin":"#35d49a"} as React.CSSProperties} onClick={()=>onSelect(v.id)} title={`GPS ${v.last_gps_at||""}`}><Navigation/><span>{v.plate_number} · GPS</span></button></Marker>)}
